...
async def receive_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in checker_sessions:
        ...
        if checker.get("resolve_as") == "visitor":
            ...
            if checker.get("resolve_visitor_name") is None:

                checker["resolve_visitor_name"] = typed
                ...
                checker.setdefault("pending_visitors", []).append({
                    "name": checker.pop("resolve_visitor_name"),
                    "from": typed,
                    "source": "Onsite",
                })

                checker["resolve_as"] = None

                await update.message.reply_text("✅ Visitor added (Onsite).")

                still_pending = await advance_checker_resolve_queue(update.message.reply_text, checker)
        ...
        if checker.get("resolve_as") == "newcomer":
            ...
            if checker.get("resolve_newcomer_name") is None:

                checker["resolve_newcomer_name"] = typed

                await update.message.reply_text(
                    f"Department for \"{typed}\"?",
                    reply_markup=build_department_picker("cndept")
                )
...
def build_checker_resolve_keyboard(group):
    ...
    keyboard = [
        [InlineKeyboardButton("👋 Mark as Visitor", callback_data="cvisitor")],
        [InlineKeyboardButton("🌱 Mark as Newcomer", callback_data="cnewcomer")],
    ]
...
async def finish_checker_report(context, checker_id, checker, result, reply_func):
    ...
    pending_visitors = checker.pop("pending_visitors", [])
    pending_newcomers = checker.pop("pending_newcomers", [])

    session["visitors"].extend(pending_visitors)
    session["newcomers"].extend(pending_newcomers)

    reported_count += len(pending_visitors) + len(pending_newcomers)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "cvisitor":
        ...
        checker["resolve_as"] = "visitor"

        await query.message.reply_text(
            f"Enter the visitor's correct name (OCR read: \"{checker.get('resolving_text', '')}\"):"
        )
    ...
    if action == "cnewcomer":
        ...
        checker["resolve_as"] = "newcomer"

        await query.message.reply_text(
            f"Enter the newcomer's correct name (OCR read: \"{checker.get('resolving_text', '')}\"):"
        )

        return

    if action.startswith("cndept:"):
        ...
        name = checker.pop("resolve_newcomer_name", None)
        checker.pop("resolving_text", None)

        if name:

            checker.setdefault("pending_newcomers", []).append({
                "name": name,
                "department": department,
                "source": "Onsite",
            })
            ...
            await query.edit_message_text(
                f"✅ Newcomer added: {name} ({department}, Onsite)"
            )
